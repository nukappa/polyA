...
def simulate_reads(genes, pAi, f_size, f_prob, reads_per_gene=100, pAlen=42,
    ...
    for gene in genes:
        ...
        n_to_simulate = int(reads_per_gene / (pAlen - min_offset + 1))
        n_simulated = n_to_simulate * (pAlen - min_offset + 1)
        pAoffsets[gene][0:n_simulated] = \
            np.concatenate([[offset] * n_to_simulate
                            for offset in range(min_offset, pAlen + 1)])
        pAoffsets[gene][n_simulated:reads_per_gene] = \
            np.random.randint(min_offset, pAlen + 1, size=reads_per_gene - n_simulated)
